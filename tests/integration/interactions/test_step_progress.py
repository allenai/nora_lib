...
def _spr(iservice: InteractionsService) -> StepProgressReporter:
    message = _msg("Hi")
...
class TestStepProgressReporter(unittest.TestCase):
    ...
    def test_create_child_step(self):
        ...
        self.assertEqual(child_spr.step_progress.parent_step_id, spr.step_progress.step_id)

    def test_with_context_management_success(self):
        with _spr(self.iservice) as spr:
            spr.start()
